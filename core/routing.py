...
not Clarabel's. Silently routing a MIQP to the wrong backend would either error
deep in the solver or, worse, return a wrong answer, so an unavailable solver
raises a clear, actionable error naming the missing solver and how to install
...
from core.ir import MeanVariance, MinCVaR, MinVariance, Objective, PortfolioSpec
...
_INSTALL_HINTS: dict[str, str] = {
    ...
    cp.SCIP: (
        "SCIP (the MIQP backend) is not installed. A cardinality limit combined "
        "with a variance/mean-variance objective is a mixed-integer *quadratic* "
        "program, which needs SCIP. Install it with `pip install pyscipopt` "
...
class SolverChoice:
    ...
    Attributes:
        ...
        name: Human-readable solver name for the report/echo
            (``"Clarabel"`` / ``"HiGHS"`` / ``"SCIP"``).
        problem_form: ``"continuous-convex"`` / ``"MILP"`` / ``"MIQP"`` — the
            label the chat loop uses to warn the user.
    """
    ...
    def is_mip(self) -> bool:
        """True for the mixed-integer forms (MILP / MIQP)."""
        return self.problem_form in {"MILP", "MIQP"}


def route_for(problem_class: ProblemClassImpact, objective: Objective) -> SolverChoice:
    """Pure routing decision from ``(problem_class, objective)``.
...
def select_solver(spec: PortfolioSpec) -> SolverChoice:
    ...
    """
    choice = route_for(spec.problem_class, spec.objective)
